...
import { STAFF_ROLES, getDefaultRate, getRateType } from '@/lib/staffRoles';
...
export function EventStaffTab({ event, onUpdate }: EventStaffTabProps) {
  ...
  const [showExtraHoursForm, setShowExtraHoursForm] = useState<string | null>(null); // Staff name
              ...
              {event.staff.map((person, index) => {
                ...
                return (
                  <div key={index} className="p-4 border rounded-lg">
                    ...
                    {/* Extra Hours within this staff member */}
                    {personExtraHours.length > 0 && (
