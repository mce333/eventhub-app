...
export function EventExpensesTab({ event, onUpdate }: EventExpensesTabProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Sección VERDURAS - Solo para encargado de compras y admin */}
      {!isCoordinador && (
        <Card className="bg-gradient-to-br from-green-500/5 to-green-500/10 border-green-500/20">
          <CardHeader className="pb-3">
            <div className="flex items-center justify-between">
              <CardTitle className="text-base">Verduras</CardTitle>
              {canEdit && (
                <Button
                  onClick={() => setShowAddVegetable(true)}
                  size="sm"
          ...
          <CardContent className="space-y-4">
            {/* Lista de verduras agregadas pendientes de guardar */}
            {selectedVegetables.length > 0 && (
              <div className="space-y-2 p-3 bg-background rounded-lg border">
                <Label className="text-sm font-medium">Verduras Agregadas (Pendientes de Registro)</Label>
                {selectedVegetables.map((veg, index) => (
                  <div key={index} className="flex items-center justify-between p-2 bg-green-500/5 rounded border">
                    <div className="flex-1">
                      <p className="font-medium text-sm">{veg.name}</p>
                      <p className="text-xs text-muted-foreground">
                        {veg.kg} kg × S/ {veg.pricePerKg}/kg
                      </p>
                    </div>
                ...
                <div className="flex justify-between items-center pt-2 border-t">
                  <p className="font-semibold">Total Verduras:</p>
                  <p className="text-lg font-bold text-green-600">
                    S/ {selectedVegetables.reduce((sum, v) => sum + v.total, 0).toFixed(2)}
                  </p>
                </div>
                <Button
                  onClick={handleSaveVegetables}
                ...
                >
                  <Save className="h-4 w-4 mr-2" />
                  Registrar Todas las Verduras
                </Button>
              </div>
            )}

            {/* Formulario para agregar verdura */}
            {showAddVegetable && (
              <div className="p-4 border-2 border-green-500/20 rounded-lg bg-green-500/5">
                <h4 className="font-semibold mb-3 text-sm">Agregar Verdura</h4>
                <div className="grid grid-cols-1 gap-3">
                  <div>
                    ...
                    >
                      <SelectTrigger>
                        <SelectValue placeholder="Selecciona una verdura" />
                      </SelectTrigger>
                      <SelectContent>
                        {VEGETABLE_OPTIONS.map((veg) => (
                          <SelectItem key={veg.name} value={veg.name}>
                    ...
                    <div className="p-2 bg-background rounded border">
                      <div className="flex justify-between text-sm">
                        ...
                        <span className="font-bold text-green-600">
                          S/ {(newVegetable.kg * newVegetable.pricePerKg).toFixed(2)}
                        </span>
                      </div>
                    </div>
                  )}
            ...
            {/* Verduras ya registradas en el evento */}
            {event.expenses?.filter(e => e.isPredetermined && e.category === 'verduras').length > 0 && (
              <div className="space-y-2 p-3 bg-background rounded-lg border">
                <Label className="text-sm font-medium">Verduras Registradas</Label>
                {event.expenses.filter(e => e.isPredetermined && e.category === 'verduras').map((expense) => (
                  <div key={expense.id} className="flex items-center justify-between p-2 bg-green-500/10 rounded border border-green-500/30">
                    <div className="flex-1">
                      <p className="font-medium text-sm">{expense.description}</p>
                      <p className="text-xs text-muted-foreground">
                        {expense.cantidad} kg × S/ {expense.costoUnitario}/kg
                      </p>
                    </div>
                    <p className="font-semibold text-green-600">S/ {expense.amount.toFixed(2)}</p>
                  </div>
                ))}
              </div>
            )}
      ...
      {/* Sección AJÍES - Solo si el plato lo requiere y no es coordinador */}
      {!isCoordinador && selectedDish && dishRequiresChili(selectedDish) && (
        <Card className="bg-gradient-to-br from-red-500/5 to-red-500/10 border-red-500/20">
          <CardHeader className="pb-3">
            <div className="flex items-center justify-between">
              <CardTitle className="text-base">Ajíes</CardTitle>
              {canEdit && (
                <Button
                  onClick={() => setShowAddChili(true)}
                  size="sm"
          ...
          <CardContent className="space-y-4">
            {/* Lista de ajíes agregados pendientes de guardar */}
            {selectedChilis.length > 0 && (
              <div className="space-y-2 p-3 bg-background rounded-lg border">
                <Label className="text-sm font-medium">Ajíes Agregados (Pendientes de Registro)</Label>
                {selectedChilis.map((chili, index) => (
                  <div key={index} className="flex items-center justify-between p-2 bg-red-500/5 rounded border">
                    <div className="flex-1">
                      <p className="font-medium text-sm">{chili.name}</p>
                      <p className="text-xs text-muted-foreground">
                        {chili.kg} kg × S/ {chili.pricePerKg}/kg
                      </p>
                    </div>
                ...
                <div className="flex justify-between items-center pt-2 border-t">
                  <p className="font-semibold">Total Ajíes:</p>
                  <p className="text-lg font-bold text-red-600">
                    S/ {selectedChilis.reduce((sum, c) => sum + c.total, 0).toFixed(2)}
                  </p>
                </div>
                <Button
                  onClick={handleSaveChilis}
                ...
                >
                  <Save className="h-4 w-4 mr-2" />
                  Registrar Todos los Ajíes
                </Button>
              </div>
            )}

            {/* Formulario para agregar ají */}
            {showAddChili && (
              <div className="p-4 border-2 border-red-500/20 rounded-lg bg-red-500/5">
                <h4 className="font-semibold mb-3 text-sm">Agregar Ají</h4>
                <div className="grid grid-cols-1 gap-3">
                  <div>
                    <Label className="text-xs">Tipo de Ají</Label>
                    <Select
                    ...
                    >
                      <SelectTrigger>
                        <SelectValue placeholder="Selecciona un ají" />
                      </SelectTrigger>
                      <SelectContent>
                        {CHILI_OPTIONS.map((chili) => (
                          <SelectItem key={chili.name} value={chili.name}>
                    ...
                    <div className="p-2 bg-background rounded border">
                      <div className="flex justify-between text-sm">
                        ...
                        <span className="font-bold text-red-600">
                          S/ {(newChili.kg * newChili.pricePerKg).toFixed(2)}
                        </span>
                      </div>
                    </div>
                  )}
            ...
            {/* Ajíes ya registrados en el evento */}
            {event.expenses?.filter(e => e.isPredetermined && e.category === 'verduras' && e.description.toLowerCase().includes('ají')).length > 0 && (
              <div className="space-y-2 p-3 bg-background rounded-lg border">
                <Label className="text-sm font-medium">Ajíes Registrados</Label>
                {event.expenses.filter(e => e.isPredetermined && e.category === 'verduras' && e.description.toLowerCase().includes('ají')).map((expense) => (
                  <div key={expense.id} className="flex items-center justify-between p-2 bg-red-500/10 rounded border border-red-500/30">
                    <div className="flex-1">
                      <p className="font-medium text-sm">{expense.description}</p>
                      <p className="text-xs text-muted-foreground">
                        {expense.cantidad} kg × S/ {expense.costoUnitario}/kg
                    ...
                    <p className="font-semibold text-red-600">S/ {expense.amount.toFixed(2)}</p>
                  </div>
                ))}
              </div>
            )}
